 ...
 */
mbo_ClientService.prototype.getAllClients = function( search ) {
	var self = this;

	return self.getClientCount()
		.then( function( clientCount ) {
			var pages = Math.ceil( clientCount / self.requestDefaults.PageSize ),
				completed = 0
				clientRequests = [];
 ...
 */
mbo_ClientService.prototype.getClientsById = function( ids ) {
	var args = {
		ClientIDs: {
			string: ids
		}
